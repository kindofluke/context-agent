"""Gradio UI for the natural language agent."""
...
from context_agent.session import _get_session_dir, _initialize_session_dir
...
def create_gradio_interface(
    ...
    Args:
        exec_dir: Agent execution directory (for single-tenant mode)
        allowed_domains: List of domains for Deno network access
        session_mode: Whether multi-tenant session mode is enabled
        sessions_base: Base directory for sessions (required if session_mode=True)
        template_path: Template directory to copy files from when initializing new sessions
        read_only_default: Default read-only mode for single-tenant

    Returns:
        Gradio Blocks instance configured with the chat interface
    """
    ...
    # Build title and description based on read-only mode
    title = "NL Agent"
    if read_only_default:
        title += " [READ-ONLY]"
        description = "Chat with your natural language agent (read-only mode: file modifications disabled)"
    else:
